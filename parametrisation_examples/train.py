...
assert (
    tie_word_embeddings != force_weight_tying
...
while True:
    ...
    if iter_num % eval_interval == 0 and master_process:
        ...
        if mup_enable_coord_check_logging and coord_check_dict is not None:
            for key, stats in coord_check_dict.items():
                if stats["count"] > 0:
                    finalized = finalize_layer_stats(stats)
                    log_dict[key + "_act_abs_mean"] = finalized["mean"]
                    log_dict[key + "_act_abs_std"] = finalized["std"]
                    log_dict[key + "_act_abs_min"] = finalized["min"]
                    log_dict[key + "_act_abs_max"] = finalized["max"]
        if wandb_log:
    ...
    if mup_enable_coord_check_logging:
        coord_check_dict = {
            "token_embedding": init_layer_stats(),
            "attn": init_layer_stats(),
            "mlp": init_layer_stats(),
            "lm_head": init_layer_stats(),
            "last_layer": init_layer_stats(),
        }

        def hook(module, input, output, key):
            with torch.no_grad():
                out = output if not isinstance(output, tuple) else output[0]
                update_layer_stats(coord_check_dict[key], out)
